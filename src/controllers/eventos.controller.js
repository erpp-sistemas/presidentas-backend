...
const newEventos = async (data) => {
    ...
    let newPosicion = 2
    let horaEvento = data.fecha_evento.split("T")[1] // 11:00
    let fechaEvento = data.fecha_evento.split("T")[0] // 2026-02-14
    const eventDate = `${fechaEvento} ${horaEvento}:00.000`
    // const eventDate = new Date(data.fecha_evento).toISOString();
    ...
    const eventos = await eventosModel.create({ ...data, posicion: 1, activo: 1, fecha_evento: eventDate })
